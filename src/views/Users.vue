      ...
      <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
		...
		<div class="list mt-4 flex flex-wrap justify-around">
			<div v-for="user in users" :key="user.id"
				class="user-item bg-light border border-primary rounded-3xl w-2/5 m-4"
			>
				<div class="flex flex-row">
					...
					<div class="self-center p-2 m-2 uppercase">
						{{user.lastname}}
					</div>
				...
				<div class="flex flex-row justify-center" >
					...
					<button role="button" class="bg-red-700 py-2 px-4 m-2 rounded-full" @click="deleteUser(user.id)">
						Supprimer
					</button>
				</div>
			</div>
		...
		updateUser(){
			fetch("http://localhost:1234/users/"+this.editingUser.id, {
				...
				body: JSON.stringify({
						...
						factures:this.editingUser.factures
				}),
				headers: {
						"Content-type": "application/json; charset=UTF-8"
				}
